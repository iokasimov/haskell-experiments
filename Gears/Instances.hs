...
instance Semigroup Base.Int where (+) = (Base.+)
instance Monoid Base.Int where zero = 0
instance Infimum Base.Int where (/\) = Base.min
instance Supremum Base.Int where (\/) = Base.max
